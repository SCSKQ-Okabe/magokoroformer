...
class KellyCriterionLoss(nn.Module):
    ...
    def forward(self, predictions, winner_idx, winner_odds, running_mask):
        ...
        # 2. ケリー基準: 実際の利益ベースで期待値を計算
        #    1 単位賭けしたときの純利益は win: odds-1, lose: -1
        #    win_prob * (f*(odds-1)) - (1-win_prob) * f = f * (win_prob * (odds-1) - (1-win_prob))
        #    つまり edge = win_prob * odds - 1.0
        win_prob = probs.gather(1, winner_idx.unsqueeze(1)).squeeze(1)
        odds = torch.clamp(winner_odds, min=1.01, max=50.0)
        net_odds = odds - 1.0

        # 期待利益のエッジ（1単位賭けあたりの純利益）
        edge = win_prob * odds - 1.0

        # ケリー投資比率 f = (p * b - q) / b = (p * (odds-1) - (1-p)) / (odds-1)
        # ただし、学習安定性のために clamp は小さく抑える
        kelly_f = torch.where(
            net_odds > 1e-8,
            (win_prob * net_odds - (1.0 - win_prob)) / (net_odds + 1e-8),
            torch.zeros_like(win_prob)
        )
        kelly_f = torch.clamp(kelly_f, min=-0.2, max=0.2)

        # 実際の期待利益を使ってケリー損失を定義
        expected_profit = kelly_f * (win_prob * net_odds - (1.0 - win_prob))
        kelly_loss = -expected_profit.mean()

        # 3. エッジを直接伸ばす補助損失（高オッズで利益が出る馬を優遇）
        #    edge > 0 のときだけ正の利得を伸ばす
        positive_edge = torch.clamp(edge, min=0.0)
        edge_loss = -positive_edge.mean()

        # 4. ランキング精度：勝ち馬が他馬より高スコアになるようにする
        #    これは離散的な argsort ではなく、微分可能な pairwise margin にする
        winner_score = predictions.gather(1, winner_idx.unsqueeze(1)).squeeze(1)
        other_mask = torch.ones_like(predictions, dtype=torch.bool)
        other_mask.scatter_(1, winner_idx.unsqueeze(1), False)

        pairwise_margin = (winner_score.unsqueeze(1) - predictions).masked_fill(~other_mask, 0.0)
        pairwise_loss = F.relu(1.0 - pairwise_margin[other_mask]).mean()

        # 5. 総損失
        #    1) CE: 正解率の確保
        #    2) Kelly: 期待利益の最大化
        #    3) Pairwise: 上位にいる馬を学習
        total_loss = (
            self.alpha * kelly_loss
            + self.beta * ce_loss
            + self.gamma * pairwise_loss
            + 0.1 * edge_loss
        )

        return total_loss, ce_loss, kelly_loss, pairwise_loss, kelly_f
...
class ImprovedHorseRacingTransformer(nn.Module):  
    ...
    def forward(self, horse_info, running_mask=None, padding_mask=None):
        ...
        # Stochastic Depth適用
        final_features = self.stochastic_depth(final_features)

        if padding_mask_expanded is not None:
            final_features = final_features * padding_mask_expanded

        # ===== 特徴統合 + Positional Encoding =====
        x = self.feature_fusion(final_features)
        x = x + self.pos_encoding[:, :num_races, :]
        ...
        horse_features = self.horse_aggregation(x)
        ...
        # ===== 予測 =====
        predictions = self.predictor(horse_features).squeeze(-1)
...
def kelly_train_model_improved_with_realtime_plot(model, train_loader, val_loader, epochs=MAX_EPOCHS, lr=0.001,
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (horse_info, winner_idx, odds, running_mask, padding_mask) in enumerate(train_loader):
            ...
            # 1単位賭けあたりの純利益: 的中なら (odds-1) * f, 外れなら -f
            batch_profit = (is_correct * (odds - 1.0) * kelly_fractions) - ((1.0 - is_correct) * kelly_fractions)
            train_kelly_recovery += batch_profit.sum().item()

        model.eval()
        val_loss = 0
        val_correct = 0
        val_total = 0
        val_kelly_recovery = 0
        ...
        with torch.no_grad():
            for horse_info, winner_idx, odds, running_mask, padding_mask in val_loader:
                ...
                is_correct = (predicted == winner_idx).float()
                batch_profit = (is_correct * (odds - 1.0) * kelly_fractions) - ((1.0 - is_correct) * kelly_fractions)
                val_kelly_recovery += batch_profit.sum().item()
        ...
        # ケリー基準による回収率（純利益/賭け金単位）
        train_kelly_rec_rate = 100 * (1.0 + train_kelly_recovery / train_total) if train_total > 0 else 0
        val_kelly_rec_rate = 100 * (1.0 + val_kelly_recovery / val_total) if val_total > 0 else 0
...
if __name__ == "__main__":
    ...
    top5_indices = torch.argsort(all_predictions, dim=1, descending=True)[:, :5]

    for top_n in range(1, 6):
        top_n_horses = top5_indices[:, :top_n]
        winners_expanded = all_winners.unsqueeze(1).expand(-1, top_n)
